...
namespace UnrealM
{
	...
	public class ActionSequence
	{
		...
		internal static ActionSequence Get() => new ActionSequence();

		internal static ActionSequence GetInstance(Component component = null) => opNodes.Get().Start(component);
		...
		{
			if (hasID && id == callerID) { Stop(); }
		}

		//外部调用停止，内部执行的时候才会自杀，为了保持运行列表与缓存池同步
		internal void Stop() { bSetStop = true; }
		...
		internal void Release()
		{
			...
			for (int i = 0; i < nodes.Count; i++) { nodes[i].Release(); }

			nodes.Clear();
		}

		internal void UpdateTimeAxis(float deltaTime) { timeAxis += deltaTime; }
		...
		internal ActionSequence SetHandle(ActionSequenceHandle handle)
		{
			...
			{
				Debug.LogWarning(
					"try set handle, but this sequence is already controlled by a handle! Original handle lose control of this sequence and input handle take control.");
