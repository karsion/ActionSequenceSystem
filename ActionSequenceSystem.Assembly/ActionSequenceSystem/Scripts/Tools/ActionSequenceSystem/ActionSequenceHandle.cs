...
namespace UnrealM
{
    ...
    public class ActionSequenceHandle
    {
        ...
        /// </summary>
        /// <returns></returns>
        public ActionSequence SequenceUnscaled()
        {
            return ActionSequenceSystem.SequenceUnscaled().SetHandle(this);
        }
        ...
        /// <param name="action">调用的函数</param>
        /// <returns></returns>
        public ActionSequence LooperUnscaled(float interval, int loopTime, bool isActionAtStart, Action action)
        {
            return isActionAtStart ?
        ...
        /// <param name="action">调用的函数，循环次数作为参数</param>
        /// <returns></returns>
        public ActionSequence LooperUnscaled(float interval, int loopTime, bool isActionAtStart, Action<int> action)
        {
            return isActionAtStart ?
                SequenceUnscaled().Action(action).Interval(interval).Loop(loopTime) :
                SequenceUnscaled().Interval(interval).Action(action).Loop(loopTime);
